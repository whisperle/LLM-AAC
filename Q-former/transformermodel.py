...
### NEW
from ..modules.qformer_projector import EncoderProjectorQFormer
...
class MMBertForJoint(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if getattr(config, "encoder_projector", "videomlp") == "qformer":
            self.projector = EncoderProjectorQFormer(config)
            self.use_qformer = True
            print("Q-Former projector initialized (EncoderProjectorQFormer).")
        else:
            self.projector = VideoTokenMLP(config)
            self.use_qformer = False
            print("Warning: Not using Q-Former. Using default VideoTokenMLP projector.")

        self.bert = MMBertModel(config)
        self.init_weights()

    def forward(self, input_ids=None, input_video_embeds=None, attention_mask=None,
                ...
                return_dict=None, separate_forward_split=None):
        
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict

        if self.use_qformer:
            attn_mask = torch.ones(input_video_embeds.shape[:2], dtype=torch.long).to(input_video_embeds.device)
            video_tokens = self.projector(input_video_embeds, attn_mask)

            # ⚡ NEW: You can print query_tokens info here if you want
            if hasattr(self.projector, "query_tokens"):
                print(f"Forward: {self.projector.query_tokens.shape[1]} query tokens | dim {self.projector.query_tokens.shape[-1]}")
        else:
            video_tokens = self.projector(input_video_embeds)

        outputs = self.bert(
            input_ids, video_tokens, attention_mask=attention_mask,
            token_type_ids=token_type_ids, position_ids=position_ids,
            head_mask=head_mask, inputs_embeds=inputs_embeds,
            output_attentions=output_attentions, output_hidden_states=output_hidden_states,
            return_dict=return_dict, separate_forward_split=separate_forward_split,
        )
        return outputs
...
class MMBertForTokenClassification(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.classifier = nn.Linear(config.hidden_size, 779)  # 779 for COIN
        self.init_weights()
...
class MMBertForMFMMLM(BertPreTrainedModel):
    """Multi-modal MLM (Masked Language Modeling)."""
    ...
    def forward(self, input_ids=None, input_video_embeds=None, attention_mask=None,
                token_type_ids=None, position_ids=None, head_mask=None, inputs_embeds=None,
                masked_frame_labels=None, target_video_hidden_states=None,
                non_masked_frame_mask=None, masked_lm_labels=None,
                output_attentions=None, output_hidden_states=None, return_dict=None):
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
        ...
        if masked_frame_labels is not None and masked_lm_labels is not None:
            ...
            mfm_scores, prediction_scores = self.cls(
                ...
                video_tokens.masked_select(non_masked_frame_mask.unsqueeze(-1)).view(-1, hidden_size),
...
class MMBertForMTM(MMBertForMFMMLM):
    """Multi-Token Matching (MTM) model."""
