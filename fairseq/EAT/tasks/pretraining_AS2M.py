...
class MaeImagePretrainingConfig(FairseqDataclass):
    ...
    # HuggingFace AudioSet parameters
    use_huggingface: bool = field(default=False, metadata={"help": "if true, use HuggingFace dataset instead of local files"})
    hf_dataset_config: str = field(default="balanced", metadata={"help": "HuggingFace dataset configuration ('balanced' or 'unbalanced')"})
    hf_cache_dir: Optional[str] = field(default=None, metadata={"help": "Directory to cache HuggingFace datasets"})
    
    # Feature type parameters
    feature_type: str = field(default="melspec", metadata={"help": "Type of audio features to use ('melspec' or 'cwt')"})
    cwt_scales: int = field(default=64, metadata={"help": "Number of scales for CWT"})
    cwt_width: float = field(default=8.0, metadata={"help": "Width parameter for Morlet wavelet"})
    
    # Precomputed features parameters
    use_precomputed_features: bool = field(default=True, metadata={"help": "Whether to use pre-computed features (for CWT)"})
    precomputed_features_path: Optional[str] = field(default=None, metadata={"help": "Path to pre-computed features file"})
    force_compute: bool = field(default=False, metadata={"help": "Force computing features even if precomputed file exists"})
