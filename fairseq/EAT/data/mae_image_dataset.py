...
from .raw_audio_dataset import FileAudioDataset, PrecomputedCWTAudioDataset
...
class MaeImageDataset(FairseqDataset):
    def __init__(
        ...
        hf_dataset_config: str = "balanced",
        ...
        if self.audio_mae:
            ...
            # Use precomputed CWT features if requested
            if self.feature_type == "cwt" and self.use_precomputed_features:
                logger.info(f"Using PrecomputedCWTAudioDataset for {split} split")
                self.dataset = PrecomputedCWTAudioDataset(
                    sample_rate=32000 if not self.downsr_16hz else 16000,
            ...
            # Use HuggingFace dataset
                ...
                manifest_path = f"{hf_dataset_config}_{split}"
                logger.info(f"Using HuggingFace dataset: {manifest_path}")
            ...
            # Use standard file-based dataset
            else:
                manifest_path = os.path.join(root, "{}.tsv".format(split))     
                logger.info(f"Using file-based dataset: {manifest_path}")
                self.dataset = FileAudioDataset(
                    manifest_path=manifest_path,
                    sample_rate=32000,
                    max_sample_size=325000,
