...
def model_factory(train_config, model_config, **kwargs):
    ...
    # If no encoder, make sure we don't try to print its size
    if encoder is None:
        logger.info("No encoder is used, will use features directly")
...
class slam_model_aac(slam_model):
    ...
    def forward(self,
        ...
        if audio_mel is not None:
            ...
            # logger.info(f"audio_mel shape after unsqueeze: {audio_mel.shape}")
        ...
        if audio_mel is not None or audio is not None:
            ...
            if self.model_config.encoder_name == "eat":
                encoder_outs = self.encoder.extract_features(audio_mel, padding_mask = None, mask=False, remove_extra_tokens = False)['x']
            ...
            if self.encoder is None:
                ...
                # logger.info(f"encoder_outs shape (when encoder is None): {encoder_outs.shape}")
                
                # For H5 datasets with preprocessed features, we need to ensure proper shape
                # Q-former expects shape [batch_size, seq_len, dim]
                if len(encoder_outs.shape) == 4 and encoder_outs.shape[1] == 1:
                    # If shape is [B, 1, T, F], reshape to [B, T, F]
                    encoder_outs = encoder_outs.squeeze(1)
                    # logger.info(f"Squeezed encoder_outs shape: {encoder_outs.shape}")
                    
                # Create audio_mel_post_mask if it's None (needed for Q-former)
                if audio_mel_post_mask is None:
                    # Create a mask that marks all positions as valid (no padding)
                    audio_mel_post_mask = torch.ones(
                        (encoder_outs.shape[0], encoder_outs.shape[1]), 
                        dtype=torch.float32, 
                        device=encoder_outs.device
                    )
                    # logger.info(f"Created audio_mel_post_mask with shape: {audio_mel_post_mask.shape}")
    ...
    def inference(
        ...
        **kwargs,
        ...
        if os.path.exists(wav_path):
            ...
            if self.model_config.encoder_name == "eat":
                encoder_outs = self.encoder.extract_features(audio_mel.unsqueeze(dim=1), padding_mask = None, mask=False, remove_extra_tokens = False)['x']
