...
def setup_encoder(train_config, model_config, **kwargs):
    ...
    if len(encoder_list) == 0 or encoder_list[0].lower() == "none":
        # Return None immediately if no encoder is specified or if 'none' is specified
        return None
    ...
    # Only print size if encoder is not None
    if encoder_list and encoder_list[0].lower() != "none":
