...
def train(model, train_dataloader,eval_dataloader, tokenizer, optimizer, lr_scheduler, gradient_accumulation_steps, train_config, log_config, fsdp_config=None, local_rank=None, rank=None):
    ...
    for epoch in range(train_config.num_epochs):
        ...
        # Conditionally use Join context manager only when distributed training is enabled
        if train_config.enable_fsdp or train_config.enable_ddp:
            ctx_managers = MemoryTrace(), Join([model, optimizer])
        else:
            ctx_managers = MemoryTrace(), nullcontext()
            
        with ctx_managers[0], ctx_managers[1]:  # track the memory usage
            model.train()
            total_loss = 0.0
            total_acc = 0.0
        ...
        if train_config.enable_fsdp:
            if rank==0:
                logger.info(f"Max CUDA memory allocated was {ctx_managers[0].peak} GB")
                logger.info(f"Max CUDA memory reserved was {ctx_managers[0].max_reserved} GB")
                logger.info(f"Peak active CUDA memory was {ctx_managers[0].peak_active_gb} GB")
                logger.info(f"Cuda Malloc retires : {ctx_managers[0].cuda_malloc_retires}")
                logger.info(f"CPU Total Peak Memory consumed during the train (max): {ctx_managers[0].cpu_peaked + ctx_managers[0].cpu_begin} GB")
        else:
            logger.info(f"Max CUDA memory allocated was {ctx_managers[0].peak} GB")
            logger.info(f"Max CUDA memory reserved was {ctx_managers[0].max_reserved} GB")
            logger.info(f"Peak active CUDA memory was {ctx_managers[0].peak_active_gb} GB")
